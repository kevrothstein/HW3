...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgres://kevinrothstein@localhost/kevinmrHW3"
## Provided:
...
class TweetEntryForm(FlaskForm):
    text = StringField('Enter the text of tweet (cannot be more than 280 characters): ', validators=[Required()])
    username = StringField('Enter the twitter username (no more than 64 characters and cannot start with @): ', validators=[Required()])
    display_name = StringField('Enter display name for the twitter user (has to be at least 2 words): ', validators=[Required()])
    submit = SubmitField()
    def validate_username(self, field):
        if '@' in field.data:
            raise ValidationError('cannot begin with @ sybol')
    def validate_display_name(self, field):
        if len(field.data.split())<2:
            raise ValidationError('Error. Needs at least 2 words')
...
def see_all_tweets():
    ...
    all_tweets = []
    for kevin in tweets:
        all_tweets.append([kevin.text, User.query.filter_by(id = kevin.user_id).first().username])
...
def get_longest_tweet():
    ...
    for kevin in all_tweets:
        l = 0
        for cheese in kevin.text:
            if cheese != ' ':
                l += 1
        if l > l_tweet:
            l_tweet = l
            longest_tweet = kevin
            user = User.query.filter_by(id = kevin.user_id).first()
